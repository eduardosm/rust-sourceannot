...
pub(super) fn build_with_char_iter<const UTF: u8>(
    ...
    loop {
        #[inline]
        fn char_orig_len<const UTF: u8>(c: char) -> usize {
            match UTF {
                0 => 1,
                8 => c.len_utf8(),
                16 => c.len_utf16(),
        ...
        if cur_chr == '\r' && next_chr == Some('\n') {
            ...
            match (cur_chr, control_char_style) {
                ('\u{00}'..='\u{1F}', ControlCharStyle::Replacement) => {
                    let replacement = char::try_from(u32::from(cur_chr) + 0x2400).unwrap();
                ...
                ('\u{00}'..='\u{FF}', _) if UTF == 0 => {
                    ...
                    builder.push_fmt(
                        format_args!("<{:02X}>", u32::from(cur_chr)),
                        orig_len,
                        control_char_alt,
                    );
                }
                ...
                ('\u{0000}'..='\u{D7FF}' | '\u{E000}'..='\u{FFFF}', _) if UTF == 16 => {
                    // Single-word UTF-16 character
                    builder.push_fmt(
                        format_args!("<{:04X}>", u32::from(cur_chr)),
                        orig_len,
                        control_char_alt,
                    );
                }
