...
//! are replaced, and invalid UTF-8 sequences can be rendered as `�` or `<XX>`).
//! The library keeps the mapping so that spans still line up with what's shown.
//!
//! # Output flexibility
//!
...
//!     true,
//!     sourceannot::InvalidUtf8SeqStyle::Replacement,
//!     true,
//! );
//!
...
//! let mut annotations = sourceannot::Annotations::new(&snippet, 1, main_style);
//!
...
pub trait Output<M> {
    ...
    ///
    /// For example, it could be [`std::io::Error`] when writing to an I/O
...
///
/// This controls how the snippet *frame* is drawn (margin, connector lines,
/// corners) and which metadata is attached to each text fragment.
///
...
pub struct MainStyle<M> {
    ...
    /// Character used to draw the horizontal connector of multi-line annotations.
